  ...
  useEffect(() => {
    ...
    if (err) {
      ...
      if (window.opener && !window.opener.closed) {
        try { window.opener.postMessage({ type:"OAUTH_ERROR", error:msg }, window.location.origin); } catch(e){}
        window.close();
        return;
      }
      // Try BroadcastChannel for PWA
      try {
    ...
    // Popup flow — postMessage already sent by the server HTML page
    if (window.opener && !window.opener.closed) {
      window.close();
      return;
    }

    // Fallback: direct navigation (no popup, no opener)
    if (ready === "1") {
