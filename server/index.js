...
// Required for correct IP/protocol behind Render's reverse proxy
...
app.use(session({
  ...
  cookie: {
    secure:   isProd,           // true on Render (HTTPS), false in dev (HTTP)
    sameSite: isProd ? "none" : "lax",  // "none" required for cross-origin HTTPS
    maxAge:   15 * 60 * 1000,   // 15 min — only needed during OAuth redirect
    httpOnly: true,
  },
}));
