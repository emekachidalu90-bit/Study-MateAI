...
// Shared OAuth handler used by both Auth page and Landing page
export function useOAuthLogin() {
  ...
  const handleOAuth = (provider) => {
    ...
    const finishLogin = async (token) => {
      cleanup();
      try {
        // Set header BEFORE calling refreshUser — this was the Chrome bug
    ...
    // ── Mechanism 1: window.opener postMessage ──
    const messageHandler = (event) => {
      if (event.origin !== window.location.origin) return;
      if (event.data?.type === "OAUTH_SUCCESS") finishLogin(event.data.token);
      else if (event.data?.type === "OAUTH_ERROR") finishError(event.data.error);
    };
    window.addEventListener("message", messageHandler);

    // ── Mechanism 2: BroadcastChannel (PWA standalone) ──
    let bc = null;
    try {
      ...
      // BroadcastChannel not supported — fine, postMessage covers it
    }
    ...
    // Poll for popup closure (user dismissed without completing)
  ...
  return (
    <div style={{ minHeight:"100vh", display:"flex", background:"var(--bg-primary)", position:"relative", overflow:"hidden" }}>
      ...
      <div className="hide-mobile" style={{ width:"42%", minHeight:"100vh", background:"linear-gradient(145deg,rgba(108,99,255,0.1),rgba(255,107,157,0.05))", borderRight:"1px solid var(--border-light)", display:"flex", flexDirection:"column", justifyContent:"center", padding:"48px 56px" }}>
        ...
        {[["📄","Upload PDF, PPTX, DOCX, Excel, TXT"],["🃏","AI flashcards & summaries in one click"],["🎮","Live multiplayer quizzes like Kahoot"],["🤖","Chat with your personal AI study tutor"],["🏆","Earn XP, level up, climb the leaderboard"]].map(([e,t]) => (
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:10, marginBottom:20 }}>
                {enabled.map(({ key, label, Icon, bg, color, border }) => (
                  <button key={key} onClick={() => handleOAuth(key)} disabled={busy}
                    style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:10, width:"100%", padding:"12px 20px", background:bg, color, border:`1.5px solid ${border}`, borderRadius:"var(--radius-sm)", fontFamily:"var(--font)", fontSize:"0.9rem", fontWeight:600, cursor:busy?"not-allowed":"pointer", transition:"all 0.18s", boxShadow:"0 2px 8px rgba(0,0,0,0.15)", opacity:busy&&loadingProvider!==key?0.6:1 }}
