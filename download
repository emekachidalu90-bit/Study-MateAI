...
import { Zap, TrendingUp, FileText, Layers, Award, Flame, Edit3, Check, X, Link2, ShieldCheck, KeyRound } from "lucide-react";
...
export default function Profile() {
  ...
  return (
    <div className="page" style={{ paddingTop:32,maxWidth:820 }}>
        ...
        <div style={{ padding:"0 26px 26px",marginTop:-38 }}>
          <div className="flex-between" style={{ flexWrap:"wrap",gap:12,alignItems:"flex-end" }}>
            <div style={{ position:"relative" }}>
              <div style={{ width:76,height:76,background:`linear-gradient(135deg,${userColor},${userColor}bb)`,borderRadius:18,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.8rem",fontWeight:800,color:"white",border:"3px solid var(--bg-card)" }}>
          ...
          <div style={{ marginTop:16,maxWidth:400 }}>
            <div className="flex-between" style={{ marginBottom:5 }}>
              <span style={{ fontSize:"0.76rem",color:"var(--text-muted)",fontWeight:600 }}><Zap size={10} style={{ display:"inline",verticalAlign:"middle" }}/> Level {level}</span>
      ...
      {tab==="overview" && (
        <div style={{ display:"flex",flexDirection:"column",gap:18 }}>
          <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(130px,1fr))",gap:13 }}>
            {[{ icon:Zap,color:"var(--accent-4)",label:"Total XP",val:`${xp}` },{ icon:TrendingUp,color:"var(--accent)",label:"Level",val:`${level}` },{ icon:Flame,color:"#FF6363",label:"Streak",val:`${streak}🔥` },{ icon:FileText,color:"var(--accent-5)",label:"Notes",val:`${stats.notes}` },{ icon:Layers,color:"var(--accent-2)",label:"Flashcards",val:`${stats.flashcards}` },{ icon:Award,color:"var(--accent-3)",label:"Badges",val:`${unlocked.length}` }].map(({ icon:Icon,color,label,val }) => (
      ...
      {tab==="achievements" && (
        <div style={{ display:"flex",flexDirection:"column",gap:18 }}>
          {unlocked.length>0&&<div><h3 style={{ fontWeight:700,marginBottom:12,color:"var(--accent-3)" }}>✅ Unlocked ({unlocked.length})</h3><div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:11 }}>{unlocked.map(a=><div key={a.id} className="card" style={{ display:"flex",gap:13,alignItems:"center",border:"1px solid rgba(67,233,123,0.22)",background:"rgba(67,233,123,0.04)" }}><span style={{ fontSize:"1.8rem" }}>{a.icon}</span><div><div style={{ fontWeight:700,color:"var(--accent-3)",fontSize:"0.88rem" }}>{a.label}</div><div style={{ fontSize:"0.76rem",color:"var(--text-muted)" }}>{a.desc}</div><div style={{ fontSize:"0.7rem",color:"var(--accent-4)",marginTop:3 }}>+{a.xp} XP</div></div></div>)}</div></div>}
          {locked.length>0&&<div><h3 style={{ fontWeight:700,marginBottom:12,color:"var(--text-muted)" }}>🔒 Locked ({locked.length})</h3><div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:11 }}>{locked.map(a=><div key={a.id} className="card" style={{ display:"flex",gap:13,alignItems:"center",opacity:0.45 }}><span style={{ fontSize:"1.8rem" }}>{a.icon}</span><div><div style={{ fontWeight:700,fontSize:"0.88rem" }}>{a.label}</div><div style={{ fontSize:"0.76rem",color:"var(--text-muted)" }}>{a.desc}</div></div></div>)}</div></div>}
        </div>
      )}
      ...
      {tab==="settings" && (
        <div style={{ display:"flex",flexDirection:"column",gap:15,maxWidth:500 }}>
          <div className="card" style={{ padding:20 }}>
            <h3 style={{ fontWeight:700,marginBottom:14,fontSize:"0.93rem",display:"flex",alignItems:"center",gap:8 }}><KeyRound size={15} color="var(--accent-2)"/> Account</h3>
            <div style={{ display:"flex",flexDirection:"column",gap:11 }}>
              <div><label className="label">Display Name</label><input className="input" value={nameInput} onChange={e=>setNameInput(e.target.value)}/></div>
              <div><label className="label">Email</label><div style={{ position:"relative" }}><input className="input" value={user?.email||"No email set"} disabled style={{ opacity:0.55,paddingRight:80 }}/>{user?.email&&<span style={{ position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",fontSize:"0.68rem",color:"var(--accent-3)",fontWeight:700,display:"flex",alignItems:"center",gap:3 }}><ShieldCheck size={11}/>Verified</span>}</div></div>
              <button className="btn btn-primary btn-sm" style={{ alignSelf:"flex-end",gap:6 }} onClick={()=>toast.success("Saved!")}><Check size={13}/> Save</button>
            </div>
          </div>

          <div className="card" style={{ padding:20 }}>
            <h3 style={{ fontWeight:700,marginBottom:6,fontSize:"0.93rem",display:"flex",alignItems:"center",gap:8 }}><Link2 size={15} color="var(--accent-5)"/> Connected Accounts</h3>
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:9 }}>
              {PROVIDERS.map(({ key,label,Icon,color,bg,border }) => {
                ...
                return (
                  <div key={key} style={{ display:"flex",alignItems:"center",gap:13,padding:"11px 15px",background:connected?bg:"var(--bg-secondary)",border:`1.5px solid ${connected?border:"var(--border-light)"}`,borderRadius:"var(--radius-sm)",transition:"all 0.2s" }}>
