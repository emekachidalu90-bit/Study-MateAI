...
export default function Landing() {
  ...
  const handleOAuth = (key) => {
    ...
    const popup = window.open(`/api/auth/${key}`, `oauth_${key}`, `width=${w},height=${h},left=${left},top=${top},toolbar=no,menubar=no`);
    if (!popup) { setLoadingProvider(null); window.location.href = `/api/auth/${key}`; return; }
    const handler = async (event) => {
      ...
      if (event.data?.type === "OAUTH_SUCCESS") {
        ...
        try {
          ...
          if (user) { toast.success(`Welcome, ${user.name?.split(" ")[0]}! 🎉`); navigate("/dashboard"); }
    ...
    const poll = setInterval(() => { if (popup.closed) { clearInterval(poll); window.removeEventListener("message", handler); setLoadingProvider(null); } }, 500);
  };
    ...
    { key:"discord", Icon:DiscordIcon, label:"Discord", bg:"#5865F2", color:"white", border:"#5865F2" },  ].filter(s => providers[s.key]);
    ...
    <div style={{ minHeight:"100vh",background:"var(--bg-primary)",overflowX:"hidden" }}>
      ...
      <nav style={{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:"16px 32px",borderBottom:"1px solid var(--border-light)",position:"sticky",top:0,background:"rgba(15,15,26,0.92)",backdropFilter:"blur(20px)",zIndex:50 }}>
        <div style={{ display:"flex",alignItems:"center",gap:10 }}>
          <div style={{ width:34,height:34,background:"linear-gradient(135deg,var(--accent),var(--accent-2))",borderRadius:9,display:"flex",alignItems:"center",justifyContent:"center" }}><BookOpen size={18} color="white"/></div>
