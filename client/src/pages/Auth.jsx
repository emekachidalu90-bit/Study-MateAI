...
// Opens OAuth in a popup — works in both browser and installed PWA
function openOAuthPopup(provider, onSuccess, onError, setLoadingProvider) {
  const w = 520, h = 620;
  ...
  if (!popup || popup.closed) {
    ...
    onError("Popup was blocked by your browser. Redirecting instead…");
    setTimeout(() => { window.location.href = `/api/auth/${provider}`; }, 1500);
    return;
  }
  ...
  const handler = (event) => {
    ...
    if (event.data?.type === "OAUTH_SUCCESS") {
      ...
      clearInterval(pollTimer);
      setLoadingProvider(null);
      onError(event.data.error || "Sign-in failed.");
    }
  };
...
export default function Auth({ mode }) {
  ...
  const handleOAuth = (provider) => {
    ...
    openOAuthPopup(
      provider,
      // onSuccess — token received via postMessage
      async (token) => {
        try {
      ...
      <div style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", padding:24 }}>
        <div style={{ width:"100%", maxWidth:420 }} className="animate-fade">
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:10, marginBottom:20 }}>
                {enabled.map(({ key, label, Icon, bg, color, border }) => (
                  <button key={key} onClick={() => handleOAuth(key)}
                    ...
                    style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:10, width:"100%", padding:"12px 20px", background:bg, color, border:`1.5px solid ${border}`, borderRadius:"var(--radius-sm)", fontFamily:"var(--font)", fontSize:"0.9rem", fontWeight:600, cursor:loadingProvider?"not-allowed":"pointer", transition:"all 0.18s", boxShadow:"0 2px 8px rgba(0,0,0,0.15)", opacity:loadingProvider&&loadingProvider!==key?0.6:1 }}
                    onMouseEnter={e => { if (!loadingProvider) { e.currentTarget.style.transform="translateY(-1px)"; e.currentTarget.style.boxShadow="0 6px 20px rgba(0,0,0,0.25)"; } }}
          ...
          <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:14 }}>
            ...
            <button type="submit" className="btn btn-primary btn-lg" style={{ marginTop:4 }} disabled={loading || !!loadingProvider}>
              {loading
