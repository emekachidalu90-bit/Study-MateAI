...
export default function Auth({ mode }) {
  ...
  useEffect(() => {
    ...
    if (err) {
      const msgs = {
        google_failed:"Google sign-in failed.", github_failed:"GitHub sign-in failed.",
        discord_failed:"Discord sign-in failed.", oauth_failed:"Social sign-in failed.",
  ...
  return (
    <div style={{ minHeight:"100vh",display:"flex",background:"var(--bg-primary)",position:"relative",overflow:"hidden" }}>
      ...
      {/* Left panel */}
      <div style={{ width:"42%",minHeight:"100vh",background:"linear-gradient(145deg,rgba(108,99,255,0.1),rgba(255,107,157,0.05))",borderRight:"1px solid var(--border-light)",display:"flex",flexDirection:"column",justifyContent:"center",padding:"48px 56px" }} className="hide-mobile">
        <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:36 }}>
          <div style={{ width:44,height:44,background:"linear-gradient(135deg,var(--accent),var(--accent-2))",borderRadius:12,display:"flex",alignItems:"center",justifyContent:"center" }}><BookOpen size={24} color="white"/></div>
        ...
        <p style={{ color:"var(--text-secondary)",lineHeight:1.7,fontSize:"0.95rem",marginBottom:32 }}>Upload notes, generate flashcards, quiz your friends live, and chat with an AI tutor.</p>
        {[["📄","Upload PDF, PPTX, DOCX, Excel, TXT"],["🃏","AI generates flashcards & summaries instantly"],["🎮","Host live multiplayer quizzes like Kahoot"],["🤖","Chat with your personal AI study tutor"],["🏆","Earn XP, level up, climb the leaderboard"]].map(([e,t]) => (
          <div key={t} style={{ display:"flex",alignItems:"center",gap:12,padding:"10px 14px",background:"rgba(255,255,255,0.03)",borderRadius:10,border:"1px solid var(--border-light)",marginBottom:8 }}>
            <span style={{ fontSize:"1.1rem",flexShrink:0 }}>{e}</span>
            <span style={{ color:"var(--text-secondary)",fontSize:"0.88rem" }}>{t}</span>
          </div>
        ))}
      </div>
      ...
      <div style={{ flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:24 }}>
        <div style={{ width:"100%",maxWidth:420 }} className="animate-fade">
          ...
          <h1 style={{ fontWeight:800,fontSize:"1.6rem",marginBottom:4 }}>{isLogin ? "Welcome back 👋" : "Create your account"}</h1>
          ...
          {enabled.length > 0 && (
            <>
              ...
              <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:20 }}>
                ...
                <span style={{ color:"var(--text-muted)",fontSize:"0.75rem",fontWeight:600,whiteSpace:"nowrap" }}>or sign in with email</span>
                <div style={{ flex:1,height:1,background:"var(--border-light)" }}/>
              </div>
            </>
          )}
